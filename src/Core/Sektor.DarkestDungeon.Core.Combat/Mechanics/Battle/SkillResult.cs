...
namespace Sektor.DarkestDungeon.Core.Combat.Mechanics.Battle
{
    ...
    public class SkillResult
    {
        ...
        public bool HasCritEffect
        {
            get
            {
                for (int i = 0; i < SkillEntries.Count; i++)
                    if (SkillEntries[i].Type == Mechanics.SkillResultType.Crit && SkillEntries[i].CanCritReleaf)
                        return true;
                return false;
            }
        }
        ...
        public void AddResultEntry(SkillResultEntry entry)
        {
            ...
            if (entry.Type != Mechanics.SkillResultType.Dodge && entry.Type != Mechanics.SkillResultType.Miss)
                HasHit = true;
            if (entry.IsZeroed)
                HasZeroHealth = true;
        }
