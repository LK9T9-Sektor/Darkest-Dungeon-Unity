...
- Рабочая линия выноса — ветка **`extraction/core`** (наследник `restructure/monorepo`, монорепо уже
  собран).
- Интеграция — **squash-merge `extraction/core` → `main`**: каждые порции работы на `extraction/core`
  вливаются в `main` одним сквош-коммитом (чистая линейная история).
- **`master` — не трогаем**: остаётся default/стабильной веткой репозитория, туда не льём.
  Открытые PR/зависимости от ветки-источника при переименовании обрываются; default — `master`.
