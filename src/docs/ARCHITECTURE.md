...
| `Content` | Классы данных контента + парсеры (JSON/DSL/CSV/XML), валидация на загрузке. Раскладка зеркалирует значимые legacy-папки: `Campaign\` — модели, `Database\` — DTO и мапперы-парсеры (первый срез: `HeirloomExchange`, `PartyNames`). DTO — PascalCase с `[JsonProperty]`; ядро ссылается на Newtonsoft.Json 13.x (присутствует в Unity), десериализация пока в адаптере презентации. Рукописные мапперы — переходное состояние; целевое состояние — прямая десериализация Newtonsoft в ядре с `[JsonProperty]` на моделях |
