...
| `Save` | Бинарный кодек + версии + хранилище (Фаза 2): `SaveCodec` (коллекции/примитивы, `SaveVersion`), `ISaveStorage` (файл↔поток — реализация в презентации), `IBinarySaveData` (контракт элементов). Полный DTO-перенос `SaveCampaignData` — вместе с Фазой 4 (зависит от кампанийных runtime-моделей); legacy `BinarySaveDataHelper`/`SaveLoadManager` делегируют коллекции/версию коду ядра |
