...
**Пак превью (только клиент, не влияет на lockstep)**: ядро применяет действие мгновенно при
получении wire-строки, а ритм показа на стороне WPF-клиента задаёт `DuelBattleViewModel`
(одна машина `PaceState` для ИИ и сети): при смене исполнителя хода — пауза 0.5с + попап «ТВОЙ ХОД»
на карточке, затем для действия соперника — превью 1с (скилл в бейдже + стрелка, либо ⇄-линия MOVE)
через `StartRivalReveal(payload)`, и только после бита применяется `ApplyRivalActionPayload`.
Паузы чисто презентационные и не меняют порядок ввода — lockstep сохраняется.
