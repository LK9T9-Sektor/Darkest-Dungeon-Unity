...
- Работа коммитится и пушится в отдельную ветку **`core/<срез>`**; **напрямую в `main` — никогда**
  (`main` защищена: force-push/удаление заблокированы, обязательные PR отключены).
- В `main` пользователь мержит сам — обычно через PR, который он создаёт сам.
