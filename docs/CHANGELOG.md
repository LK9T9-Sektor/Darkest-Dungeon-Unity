...
- ИИ соперника в дуэли теперь ходит через core-инфраструктуру `MonsterBrain`/desires
  (`DuelAi`): выбирает легальный скилл и цель с наименьшим HP (раньше — полностью случайно).
  Выбор на клиент-локальном RNG, чтобы детерминированный локстап не расходился.
  Поведение заметно только в режиме VS AI.
