...
- **Плоская инверсионная полоса**: вместо сетки 8×4 и псевдо-объёма — один ряд по центру поля (~22px). Длина зависит от рангов с учётом инверсии (слоты 0..7 = герои 4,3,2,1 \| монстры 1,2,3,4): «герой pos1 → монстр pos1» — короткий сегмент между фронтами, «герой pos4 → монстр pos4» — через всё поле; подсвечена полоса за колонкой актора включительно до колонки цели.
- **Точное выравнивание**: колонки полосы калибруются один раз при первом layout по фактическим карточкам (замер на старте, далее ресайз окна — ровное масштабирование Viewbox), а не по статичной сетке.
