...
### Unity BattleTest: сцена гибкого боя на чистом ядре (первый потребитель ядра в Unity)

- **Новая сцена `Assets\Scenes\BattleTest.unity`** (в `unity\` и `unity-2017\`): настраиваемый бой
  (герои/монстры по слотам, режимы PLAYER/AI · AI vs AI · HOTSEAT, seed, torch) на чистом ядре
  (`Core.Combat`/`Core.Duel`, `DuelController`) с легаси-префабами юнитов (Spine). Unity — тонкий
  вид-слой: `CoreBattleDriver`, формации, HP/стресс, попапы, экран боя. Сцена пересобирается
  `tools\unity-generate-battle-test-scene.ps1`; легаси-движок (`RaidSceneManager`) не используется
  и не изменён.
- **Ядро PvE-сверка**: sweep-тест по всем монстрам кампании (`FightPveSweepTests`) — 225 монстров
  добиваются боя без исключений и детерминированно. Закрыт краш `.disease` (NRE при пустом пуле
  болезней); конкретные id болезней (`the_worries`, `rabies`) теперь парсятся и применяются
  (`IBattleContext.GetQuirk` → `Hero.AddQuirk`). Рандомный пул болезней — стаб.

### WPF: монстры и боссы размером 1–4 ранга + PvE-режим (герои против монстров)

- **Size-aware ранги в ядре** (паритет Unity): ранг юнита назначается кумулятивно по `size`
  (`FormationParty`, `TurnMover`, `SurpriseResolver`, `DuelBattleEvents.MoveUnit`) — size-2 монстр
  занимает ранги 1–2, следующий юнит стартует с ранга 3. Раньше ранги были по индексу (перекрытие
  для больших монстров). Формация — 4 слота (`Σ size ≤ 4`).
- **WPF-вид**: карточка юнита рендерится шириной `185 × size` и занимает N слотов; монстр с size 2–4
  виден «крупным» на своей половине.
- **PvE-режим**: кнопка PvE в главном меню → лобби (4 героя + слоты монстров из каталога, seed,
  Σ size ≤ 4) → бой на чистом ядре `StartFight`: герои игрока ходят, монстры — своими кампанийными
  брейнами, победа/поражение/ничья с баннером.

### WPF-дуэль: формации привязаны к своим половинам (смерти не двигают команды)

- **Боевое поле больше не «схлопывается» при смертях**: команды закреплены за своими половинами с
  фиксированным зазором по центру — герои (ранги 4→1, фронт у центра) справа-выравнивание, враги
  (1→4) слева-выравнивание. Когда юнит умирает, выжившие на его половине сдвигаются к центру
  (рефлоу рангов, как в ядре), но правая команда больше не съезжает влево — центр всегда разделяет
  отряды.

### WPF-дуэль: тултипы скиллов на ховер во всех местах, бейдж иконко-без-текста

- **Тултип скилла снова появляется по ховеру** на квадратиках в подсказке персонажа (ПКМ) и на бейдже
  выбранной способности. Раньше они были не «ховеряемыми» (read-only квадратик отключал хит-тест;
  весь слой стрелок был прозрачен для мыши) — теперь квадратики всегда ховеряются, а рисованные
  линии/стрелки остаются инертными.
- **Бейдж выбранной способности — единый переиспользуемый контрол** (`SkillSquareView`, `ShowName=false`):
  только иконка без текста внутри, имя — в тултипе по ховеру; некликабельный (чистый индикатор).
  Тот же контрол используется в трэе скиллов и в стат-листе — один контрол во всех трёх местах.

### WPF-дуэль: v-образная стрелка перемещения, фикс обрезки бейджа, переиспользуемый контрол скилла

- **Стрелка перемещения (move)** — двуглавая линия ⇄ между **центрами** карточки ходящего и карточки
  цели (обмен рангами с соседним союзником): стрелка у обоих концов, а не одна прямая «верх → центр»
  с единственным наконечником.
- **Бейдж выбранной способности** больше не обрезается сверху: убран лишний подъём `BadgeLift`, позиция
  клампится в границы канваса — бейдж целиком виден над карточкой.
- **Переиспользуемый контрол «квадратик скилла»** (`SkillSquareView`): квадрат с иконкой + имя снизу,
  тултип-подсказка на ховер. Применён в нижнем левом трэе скиллов (вместо инлайнового шаблона) и в
  подсказке персонажа по правой кнопке (вместо голого текста) — скиллы там теперь показываются
  квадратиками с тултипами.

### Тултип способности в WPF-дуэли: читаемое имя + рендер

- **Читаемое имя**: заголовок тултипа (и подписи/бейджа выбранной способности) показывает
  человекочитаемое имя скилла через `Ui.DisplayNames.Title(skill.Id)` («Smite», «Zealous Accusation»),
  а не сырой id.
- **Устойчивый рендер**: корню `SkillTooltipView` задан `MaxWidth=400`, чтобы перенос текста во
  всплывающем `ToolTip` не схлопывался/не обрезался — тултип всегда крупный (бейдж уровня, имя,
  `BaseInfo`, таблица эффектов), как описано в `TESTING.md:242`.
- Новый регресс-тест `SkillTooltip_Open_RendersFully` (`RenderCaptureTests`): реально открывает
  тултип кнопки способности, проверяет DataContext контента, читаемое имя (не id), непустые
  `BaseInfo`/`EffectRows` и ненулевой размер (не «щель»).

### Тринкеты в WPF-дуэли (P1.4)

- **Тринкеты в дуэли**: у каждого героя 2 слота тринкетов; баффы тринкетов применяются как
  permanent-баффы (`BuffSourceType.Trinket`) и влияют на статы. Парсинг `JsonTrinkets` в `Core.Content`,
  применение — в `DuelController.ApplyTrinkets` (как `ApplyQuirks`), на герое пишутся
  `EquippedTrinketIds`.
- **Выбор в лобби**: у слота героя 2 стрелки-слота тринкетов (цикл + reroll по валидным для класса,
  фильтр `HeroClassRequirements`); выбранные id передаются в отряд через `DuelPartyConfig`.
- **Показ в боевом HUD**: в левой DD-панели контрол тринкетов показывает id баффывающих тринкетов
  (слоты под будущие картинки).
- **Сетевой формат**: `DuelPartyConfig` расширен 5-м `|`-полем `trinkets` (обратная совместимость со
  старыми строками из 4 полей).
- Новые тесты: `DuelTrinketTests` (бафф тринкета меняет стат, id на герое, неизвестный id
  игнорируется), `LobbySlotTests` (слоты/фильтр/reroll), round-trip конфига + legacy-строка.

### WPF-бой: механики ядра целиком + стрелка «бейдж скилла → цель»

- **АОЕ и партийные эффекты**: дуэль раскрывает цель через `BattleSolver.SelectSkillTargets` и
  исполняет скилл по каждой цели (как Unity `ExecuteSkillBase`). Hellion `breakthru` бьёт всех врагов
  в рангах, Vestal `gods_comfort` и PD `emboldening_vapours` действуют на всю партию.
- **Валидация целей**: атакующей способностью больше нельзя кликнуть себя или союзника — ядро
  отклоняет невалидную цель в `ExecuteLocalSkill`, выбор скилла сохраняется.
- **Pull/push врага починен**: корень «притяжки/отталкивания не работают» — `ProcessEventQueues`
  итерировал живой список партии, а `MoveUnit` переставлял элементы «коллекция изменена» посреди
  перечисления (падение боя). Теперь итерация по снапшоту `Units`.
- **Баффы/дебаффы на карточках**: `Character.BuffInfos` экспонирован в UI; полоска статусов
  карточки показывает активные баффы/дебаффы (id + остаток раундов).
- **Стрелка цели**: вместо полосы прямоугольников — **бейдж выбранной способности над карточкой
  действующего героя** + прямая линия из его центра в центр наведённой цели со стрелкой на конце
  (чистая математика `Ui/TargetArrowMath`, позиции через `TransformToVisual`). Move-режим — линия
  без бейджа. Оверлей прозрачен для мыши.
- Новые тесты: `DuelSkillExecutionTests` (самоклик/алли-клик отклонены, АОЕ, партийный хил,
  **одиночный хил по кликнутому союзнику**, бафф, pull), `TargetArrowMathTests`, рендер-тест
  стрелки; WPF-тест выбора целей хил-скилла (только союзники, враг игнорируется); lockstep-тесты
  остались зелёными.

### Duel HUD: стрелка «актор → цель» по ховеру (заменено выше)

Промежуточная версия стрелки — плоская полоса-«маска» из прямоугольников по центру поля
(предрасчёт 32 масок в `DuelArrowCells`, калибровка колонок по карточкам). **Заменена** на
бейдж + линию со стрелкой (см. «WPF-бой: механики ядра целиком…» выше): `DuelArrowCells`
и `ArrowGrid` удалены.
